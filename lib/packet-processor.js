...
class PacketProcessor {
  ...
  loadHomeLocation() {
    if (process.env.HOME_LATITUDE && process.env.HOME_LONGITUDE) {
      ...
      console.log(`[Home Location] Using env vars — lat: ${lat}, lon: ${lon} (${city}, ${region}, ${country})`);
      return {
        country,
        region,
        city,
  ...
  detectHomeLocation() {
    ...
    try {
      ...
      if (publicIp) {
        ...
        if (geo) {
          console.log(`[Home Location] Autodetected from public IP ${publicIp} — lat: ${geo.ll[0]}, lon: ${geo.ll[1]} (${geo.city || 'Unknown'}, ${geo.region || ''}, ${geo.country})`);
          return {
            country: geo.country,
            region: geo.region || '',
      ...
      console.warn('[Home Location] Failed to detect public IP:', err.message);
    }
  ...
  listInterfaces() {
    try {
      console.log('[PacketProcessor] Calling pcap.findalldevs()...');
      const devices = pcap.findalldevs();
      console.log('[PacketProcessor] Found interfaces:', JSON.stringify(devices, null, 2));
  ...
  startLiveCapture(iface, callback) {
    try {
      ...
      try {
        ...
        console.warn('[PacketProcessor] BPF filter failed, falling back to unfiltered capture:', filterErr.message);
