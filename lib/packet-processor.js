...
function loadPcap() {
  if (pcapModule) return pcapModule;
  if (pcapLoadError) throw pcapLoadError;

  try {
    pcapModule = require('pcap');
    return pcapModule;
  } catch (cause) {
    const error = new Error(
      'Packet capture is unavailable because the native pcap binding could not be loaded. ' +
      'Use Palo Alto Threat CSV or FTD syslog mode, or run a Node.js version compatible with pcap.'
    );
    error.code = 'PCAP_UNAVAILABLE';
    error.cause = cause;
    pcapLoadError = error;
    throw error;
  }
...
class PacketProcessor {
  ...
  listInterfaces() {
    try {
      ...
      if (err.code === 'PCAP_UNAVAILABLE') throw err;
      return [];
    }
  }
