...
class SyslogProcessor {
  ...
  startFromFile(filePath, callback) {
    ...
    rl.on('close', () => {
      ...
      if (isJsonArray && jsonArrayBuf) {
        try {
          ...
          console.error(`[SyslogProcessor] Failed to parse JSON array file: ${err.message}`);
        }
      }

      console.log(`[SyslogProcessor] Loaded ${parsed.length} syslog entries from file`);
    ...
    rl.on('error', (err) => {
      this.isRunning = false;
      console.error(`[SyslogProcessor] Failed to read syslog file: ${err.message}`);
    });
  }
